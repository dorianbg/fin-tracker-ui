...
def render():
    ...
    with settings_col:
        st.subheader("Puke Detection Settings")
        ...
        z_score_threshold = st.slider(
            ...
            key="puke_z_score_threshold",
        )

        vol_ratio_threshold = st.slider(
    ...
    with content_col:
        ...
        if drawdown_col not in df.columns:
            if "drawdown_52w" in df.columns and dd_horizon == "3Y High":
                st.warning("3Y High data not available yet. Using 52W High.")
                drawdown_col = "drawdown_52w"
                drawdown_label = "52W High"
        ...
        z_cols_available = [
            c for c in ["z_1d", "z_1w", "z_2w", "z_1mo"] if c in df.columns
        ]

        if z_cols_available:
            # find rows where any z-score exceeds threshold
            z_mask = (
                df[z_cols_available].apply(lambda row: row.max(), axis=1)
                >= z_score_threshold
            )
            ...
            if extreme.empty:
                ...
                extreme["trigger_return"] = extreme.apply(
                    lambda row: row.get(
                        z_to_return.get(row["max_z_period"], "r_1d"), 0
                    ),
                    axis=1,
                )
                ...
                is_crashing = (extreme["trigger_return"] < 0) & (
                    extreme["neg_count"] >= 2
                )
        ...
        if drawdown_col in df.columns:
            ...
            dd = (
                dd.sort_values(drawdown_col, ascending=True)
                .head(30)
                .reset_index(drop=True)
            )
            ...
            with col_dd2:
                fig_dd = px.bar(
                    ...
                    labels={
                        drawdown_col: f"% from {drawdown_label}",
                        "description": "",
                    },
        ...
        if cap_strict.empty:
            ...
            if cap_watchlist.empty:
                ...
                if "vol_ratio" not in fallback_df.columns:
                    ...
                    v1 = (
                        fallback_df["vol_1mo"]
                        if "vol_1mo" in fallback_df.columns
                        else 0
                    )
                    v12 = (
                        fallback_df["vol_1y"] if "vol_1y" in fallback_df.columns else 1
                    )
                ...
                st.dataframe(
                    top_stress[cols_f].style.format(
                        ...
                        subset=[
                            c
                            for c in cols_f
                            if pd.api.types.is_numeric_dtype(top_stress[c])
                        ],
                    ),
                    hide_index=True,
                    height=400,
                ...
                st.dataframe(
                    cap_strict[cols].style.format(
                        ...
                        subset=[
                            c
                            for c in cols
                            if pd.api.types.is_numeric_dtype(cap_strict[c])
                        ],
                    ),
                    hide_index=True,
                    height=400,
            ...
            with col_right:
                scatter_x = (
                    drawdown_col if drawdown_col in cap_strict.columns else "ma_252"
                )
                fig = px.scatter(
                    cap_strict,
                    x=scatter_x,
        ...
        if active_cap.empty and cap_watchlist.empty:
            ...
            if buys.empty:
                ...
                near_bounce = (
                    pool.sort_values("r_1w", ascending=False)
                    .head(10)
                    .reset_index(drop=True)
                )
                near_bounce.index += 1
                ...
                # We have buys!
                buys = buys.sort_values("severity", ascending=False).reset_index(
                    drop=True
                )
                buys.index += 1
                ...
                st.dataframe(
                    buys[disp_cols].style.format(
                        ...
                        subset=[
                            c
                            for c in disp_cols
                            if pd.api.types.is_numeric_dtype(buys[c])
                        ],
                    ),
                    height=350,
                )
        ...
        vol_view = (
            vol_view.sort_values("vol_ratio", ascending=False)
            .head(30)
            .reset_index(drop=True)
        )
