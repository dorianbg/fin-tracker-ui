...
def build_strategy_body(
    ...
    lines = [
        ...
        "",
        "Each alert includes the trigger, recent performance, volatility context, and 1Y/3Y charts so you can quickly decide whether it belongs on the watchlist.",
        "",
    ]
...
def chart_strategy_signals(
    ...
    for _, row in signals.reset_index(drop=True).iterrows():
        ...
        if return_lines:
            box_text = "Historical performance\n" + "   ".join(return_lines[:3])
...
def send_strategy_alerts(args: argparse.Namespace) -> None:
    ...
    for strategy in _selected(
        ...
        if not args.active_only and not changes.empty:
            ...
            _send_formatted(title, commentary, changes, prices, args.dry_run)

        if not args.changes_only and not strategy.active.empty:
        ...
        if not args.dry_run:
            save_current(state_dir, strategy.strategy_id, args.session, strategy.active)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--max-items",
        type=int,
        default=int(os.environ.get("STRATEGY_ALERT_MAX_ITEMS", "10")),
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
